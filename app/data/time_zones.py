...
def _count() -> int:
    with get_session() as session:
        return len(session.exec(select(TimeZone)).all())
...
async def bulk_create_time_zones(rows: list[dict[str, str]]) -> None:
    ...
    the shape produced by the setup wizard's CSV upload."""
    await run.io_bound(_bulk_create, rows)
